...
filter ::
  ...
  -> List a
filter _ Nil = Nil
filter p (x :. xs) = let xs' = filter p xs in
...
flatten ::
  ...
  -> List a
flatten Nil = Nil
flatten (xs :. xss) = flatten' xs xss
        where
          flatten' :: List a -> List (List a) -> List a
          flatten' Nil yss = flatten yss
          flatten' (y :. ys) yss = y :. flatten' ys yss
...
lengthGT4 ::
  List a
  -> Bool
lengthGT4 xs = lengthGT 4 xs
        where
          lengthGT _ Nil = False
          lengthGT 0 _ = True
          lengthGT l (_ :. ys) = lengthGT (l-1) ys
...
reverse ::
  ...
  -> List a
reverse Nil = Nil
reverse (x :. xs) = reverse' (x :. Nil) xs
        where
          reverse' :: List a -> List a -> List a
          reverse' acc Nil = acc
          reverse' acc (y :. ys) = reverse' (y :. acc) ys
