...
noneof ::
  Chars
  -> Parser Char
noneof str = satisfy (\c -> notElem c str)
...
betweenCharTok ::
  ...
  -> Parser a
  -> Parser a
betweenCharTok open close p = between (charTok open) (charTok close) p
...
hex ::
  Parser Char
hex = do
  hexStr <- replicateA 4 (satisfy isHexDigit)
  helper hexStr (readHex hexStr)
    where
      helper :: Chars -> Optional Int -> Parser Char
      helper hexStr Empty = constantParser (UnexpectedString hexStr)
      helper _ (Full hexVal) = return (chr hexVal)
...
eof ::
  Parser ()
eof = P helper where
  helper :: Input -> ParseResult ()
  helper Nil = Result Nil ()
  helper _ = UnexpectedEof
...
satisfyAll ::
  List (Char -> Bool)
  -> Parser Char
satisfyAll ps = satisfy helper1 where
  helper1 :: Char -> Bool
  helper1 c = and ((sequence ps) c)
...
satisfyAny ::
  List (Char -> Bool)
  -> Parser Char
satisfyAny ps = satisfy helper1 where
  helper1 :: Char -> Bool
  helper1 c = or ((sequence ps) c)
...
betweenSepbyComma ::
  ...
  -> Parser a
  -> Parser (List a)
betweenSepbyComma open close p = helper1 where
  helper1 :: Parser (List a)
  helper1 = betweenCharTok open close helper2

  helper2 :: Parser (List a)
  helper2 = sepby p (charTok ',')
